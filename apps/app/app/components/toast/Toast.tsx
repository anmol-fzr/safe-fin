...
function isReactNode(value: any): boolean {
	...
	if (value == null || primitiveTypes.includes(typeof value)) return true;
...
const getColor = (type: ToastVariant) => {
	switch (type) {
		case "success":
...
const getIconForType = (type: ToastVariant): Icon => {
	switch (type) {
		case "success":
			return TickCircle;
		case "error":
...
export const Toast = ({ toast, index }: ToastProps) => {
	...
	const color = getColor(toast.options.type);
		...
		>
			...
			>
				...
				<View style={styles.contentContainer}>
					{isReactNode(toast.content) ? (
						toast.content
					) : (
						<View>
							<Text style={[toastStyles.toastTitle, { color }]}>Title</Text>
							<Text style={toastStyles.toastDescription}>Description</Text>
						</View>
					)}
				</View>
