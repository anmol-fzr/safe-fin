...
const $customButtonTextStyle: ThemedStyle<TextStyle> = ({
	colors,
	typography,
}) => ({
	color: colors.error,
...
const $customButtonRightAccessoryStyle: ThemedStyle<ViewStyle> = ({
	colors,
}) => ({
	width: "53%",
	height: "200%",
...
const $customButtonPressedRightAccessoryStyle: ThemedStyle<ImageStyle> = ({
	colors,
}) => ({
...
export const DemoButton: Demo = {
	...
	data: ({ themed }) => [
		...
		>
			<Button
				text={translate("demoButton:useCase.passingContent.viaTextProps")}
			/>
			<DemoDivider />
		...
		>
			...
			<Button
				preset="reversed"
				RightAccessory={() => (
					<View style={themed($customButtonRightAccessoryStyle)} />
				)}
			>
